  ...
  if (clashes.length === 0) {
    return (
      <div className="flex items-center gap-[0.53rem] px-[1rem] py-[0.53rem] mb-[1rem] bg-[var(--hijau-lembut)] border border-[var(--hijau)]/25 rounded-[var(--r-sedang)] text-[0.87rem] text-[var(--hijau)]">
        ...
        Tidak ada bentrokan di {academicYearLabel}.
      </div>
    )
  }
          ...
          {clashes.map((c, i) => (
            ...
            >
              ...
              <span className="flex-1 min-w-[16rem] text-[1rem]">
                <b className="font-semibold">{c.a.nama_mk}</b> dan <b className="font-semibold">{c.b.nama_mk}</b> berjalan
                pada waktu yang sama di hari {hariLabel(c.a.hari)} &mdash; {REASON[c.type](c.detail)}.
                {c.policy === 'peringatan' && (
